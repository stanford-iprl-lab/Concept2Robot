...
class Engine58(Engine):
    def __init__(self, worker_id, opti, p_id, taskId=5, maxSteps=15, n_dmps=3, cReward=True):
        super(Engine58,self).__init__(opti, wid=worker_id, p_id=p_id, maxSteps=maxSteps, taskId=taskId, n_dmps=n_dmps, cReward=cReward,robot_model=None)
        self.opti = opti
        self._wid = worker_id
    ...
    def init_obj(self):
        self.obj_position = [0.35, -0.02, 0.24]
        self.obj_scaling = 1.0
        self.obj_orientation = self.p.getQuaternionFromEuler([.0,0.0,-math.pi/4.0])
        self.obj_id = self.p.loadURDF(os.path.join(self.urdf_dir,"obj_libs/spins/s1/s1.urdf"),basePosition=self.obj_position,baseOrientation=self.obj_orientation,globalScaling=self.obj_scaling,useFixedBase=True)
    ...
    def reset_obj(self):
        ...
        self.obj_position = np.array([self.obj_x,self.obj_y,0.24])
        self.p.resetBasePositionAndOrientation(self.obj_id,self.obj_position,self.obj_orientation)
        rotation_deg = np.random.uniform(-3.14,3.14,size=(1,)) 
        self.p.resetJointState(self.obj_id,0,rotation_deg)        

        obj_friction_ceof = 0.1
    ...
    def get_success(self,seg=None):
        degree = self.p.getJointState(self.obj_id,0)[0]
        if np.abs(degree - self.degree) > 0.1:
          return True
        else:
          return False
