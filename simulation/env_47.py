...
class Engine47(Engine):
    def __init__(self, worker_id, opti, p_id, taskId=5, maxSteps=15, n_dmps=3, cReward=True):
        super(Engine47,self).__init__(opti, wid=worker_id, p_id=p_id, maxSteps=maxSteps, taskId=taskId, n_dmps=n_dmps, cReward=cReward,robot_model=None)
        self.opti = opti
        self._wid = worker_id
    ...
    def reset_obj(self):
        self.p.resetBasePositionAndOrientation(self.obj_id,[0.3637 + 0.08, -0.07, 0.34],[0, 0, -0.1494381, 0.9887711])
    ...
    def get_success(self,seg=None):
        ...
        if len (left_closet_info) > 0 and len (right_closet_info) > 0 and obj_pos[2] > self.init_pos[2] + 0.1:
          return True
        else:
          return False
