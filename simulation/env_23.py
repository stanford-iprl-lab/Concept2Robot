...
class Engine23(Engine):
    def __init__(self, worker_id, opti, p_id, taskId=5, maxSteps=15, n_dmps=3, cReward=True):
        super(Engine23,self).__init__(opti, wid=worker_id, p_id=p_id, maxSteps=maxSteps, taskId=taskId, n_dmps=n_dmps, cReward=cReward,robot_model=None)
        self.opti = opti
        self.wid = worker_id
        self.p = p_id
    ...
    def reset_obj(self):
        self.obj_position = [0.29, 0.2, 0.45]
    ...
    def get_success (self,seg=None):
        ...
        pos1 = np.array(pos1)
        dist = np.linalg.norm(pos1-self.obj_position)
        if dist > 0.1 and pos1[1] < -0.15:
          return True
        else:
          return False
