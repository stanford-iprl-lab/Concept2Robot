...
if __name__  ==  "__main__":
  ...
  if 0:
   d_test = DDMP(opti,n_dmps=3,goal=goal_gt,start=start,force=force_gt,timesteps=T)
   f_des = d_test.imitate_path(y_gt)
   d_des = DDMP(opti,n_dmps=3,goal=goal_gt,start=start,force=f_des,timesteps=T)
   y_, dy_, ddy_  = d_des.rollout()
   

  if 0:
   d_test = DDMP(opti,n_dmps=3,goal=goal,start=start,force=force_gt,timesteps=T) 
   for i in range(100):
    y_, dy_, ddy_  = d_test.rollout()
    grad_goal = d_test.gradient_goal(y_gt)
    goal = goal - 0.01 * grad_goal
    d_test.set_goal(goal)
    print(np.linalg.norm(y_-y_gt))

  force = force_gt + np.random.uniform(0,1,size=(T,3)) * T * T

  if 0:
    d_test = DDMP(opti,n_dmps=3,goal=goal_gt, start=start,force=force,timesteps=T) 
    for i in range(100000000):
      y_, dy_, ddy_ = d_test.rollout()
      grad_force = d_test.imitate_path(y_gt)
      grad_goal = d_test.gradient_goal(y_gt)
      force = force - 0.001 * grad_force
      goal = goal - 0.01 * grad_goal
      d_test.set_force(force)
      d_test.set_goal(goal)

      print(np.linalg.norm(y_[-1]-goal_gt))
  ...
  if 1:
    ...
    for i in range(100000000):
      y_, dy_, ddy_ = d_test.rollout()
      grad_force = d_test.gradient_force_v2(y_gt)
      grad_goal = d_test.gradient_goal(y_gt)
      force = force - 50.0* grad_force
      goal = goal - 1.0 * grad_goal
      d_test.set_force(force)
    ...
    # if i % 1000 == 0:
        # print(force - force_gt)
        # print(grad_force)
        # print('a', np.expand_dims(y_ - y_gt,1).shape, np.expand_dims(y_ - y_gt, 1))
        # print('b', d_test.grad_y_force.shape, d_test.grad_y_force)
        # print(np.expand_dims(y_ - y_gt, 1) * d_test.grad_y_force)
 
  #plt.figure(0)
  #for i in range(d_test.n_dmps):
  #  plt.plot(y_[:,i])
  #  plt.plot(np.ones((lenT,))*goal[i])
  #plt.show()
