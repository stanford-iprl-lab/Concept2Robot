import copy
import numpy as np
import os
import sys
import time

np.set_printoptions(precision=4, suppress=False)

import importlib
import glob
import imageio
import math
import datetime

BASE_DIR = os.path.dirname(os.path.abspath(__file__))

import torchvision.models as models
import torch
import torch.nn as nn
import torch.nn.functional as F

from torchvision import transforms


def set_init(layers):
    for layer in layers:
        nn.init.normal_(layer.weight, mean=0., std=0.05)
        nn.init.constant_(layer.bias, 0.)


class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, task_dim, max_action, params):
        super(Actor, self).__init__()
        self.params = params
        self.model = models.resnet18(pretrained=True)
        self.action_dim = action_dim
        self.max_action = max_action
        self.feature_extractor = torch.nn.Sequential(*list(self.model.children())[:-2])
        self.img_feat_block1 = nn.Sequential(
            nn.Conv2d(in_channels=512, out_channels=256, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=True),
            nn.ReLU(),
            nn.BatchNorm2d(256),
        )
        self.img_feat_block2 = nn.Linear(256 * 2 * 3, 256)

        self.task_feat_block1 = nn.Linear(1024, 512)
        self.task_feat_block2 = nn.Linear(512, 256)
        self.task_feat_block3 = nn.Linear(256, 128)

        self.action_feat_block1 = nn.Linear(256 + 128, 256)
        self.action_feat_block2 = nn.Linear(256, 128)
        self.action_feat_block3 = nn.Linear(128, 64)
        self.action_feat_block4 = nn.Linear(64, self.action_dim)

        self.output_action_dim = 7 if self.params.haptic_net_separate else self.action_dim

        #####3 Force
        # 1
        self.force_feat_block1 = nn.Sequential(
            nn.ConvTranspose1d(in_channels=256 + 128, out_channels=256, kernel_size=4, stride=1, bias=True),
            nn.ReLU(),
        )

        # 3
        self.force_feat_block2 = nn.Sequential(
            nn.ConvTranspose1d(in_channels=256, out_channels=128, kernel_size=3, stride=2, padding=1, bias=True),
            nn.ReLU(),
        )

        # 7
        self.force_feat_block3 = nn.Sequential(
            nn.ConvTranspose1d(in_channels=128, out_channels=64, kernel_size=3, stride=2, padding=1, bias=True),
            nn.ReLU(),
        )

        #
        self.force_feat_block4 = nn.Sequential(
            nn.ConvTranspose1d(in_channels=64, out_channels=64, kernel_size=3, stride=2, padding=1, bias=True),
            nn.ReLU(),
        )

        self.force_feat_block5 = nn.Sequential(
            nn.ConvTranspose1d(in_channels=64, out_channels=self.output_action_dim, kernel_size=3, stride=2, padding=1),
        )

        layers = [self.img_feat_block2, self.task_feat_block1, self.task_feat_block2, self.task_feat_block3, \
                  self.action_feat_block1, self.action_feat_block2, self.action_feat_block3, self.action_feat_block4]

        #####3 Haptics
        if self.params.haptic_net_separate:
            # 1
            self.haptic_feat_block1 = nn.Sequential(
                nn.ConvTranspose1d(in_channels=256 + 128, out_channels=256, kernel_size=4, stride=1, bias=True),
                nn.ReLU(),
            )

            # 3
            self.haptic_feat_block2 = nn.Sequential(
                nn.ConvTranspose1d(in_channels=256, out_channels=128, kernel_size=3, stride=2, padding=1, bias=True),
                nn.ReLU(),
            )

            # 7
            self.haptic_feat_block3 = nn.Sequential(
                nn.ConvTranspose1d(in_channels=128, out_channels=64, kernel_size=3, stride=2, padding=1, bias=True),
                nn.ReLU(),
            )

            #
            self.haptic_feat_block4 = nn.Sequential(
                nn.ConvTranspose1d(in_channels=64, out_channels=64, kernel_size=3, stride=2, padding=1, bias=True),
                nn.ReLU(),
            )
            # stacked features
            self.haptic_feat_block5_cat = nn.Sequential(
                nn.ConvTranspose1d(in_channels=128, out_channels=3, kernel_size=3, stride=2, padding=1),
            )

        set_init(layers)

    def forward(self, state, task_vec):
        img_feat = self.feature_extractor(state)
        img_feat = self.img_feat_block1(img_feat)
        img_feat = img_feat.view(-1, 256 * 2 * 3)
        img_feat = self.img_feat_block2(img_feat)

        task_feat = F.relu(self.task_feat_block1(task_vec))
        task_feat = F.relu(self.task_feat_block2(task_feat))
        task_feat = F.relu(self.task_feat_block3(task_feat))

        action_feat_raw = torch.cat([img_feat, task_feat], -1)

        action_feat = F.relu(self.action_feat_block1(action_feat_raw))
        action_feat = F.relu(self.action_feat_block2(action_feat))
        action_feat = F.relu(self.action_feat_block3(action_feat))
        action_feat = self.action_feat_block4(action_feat)
        action = self.max_action * torch.tanh(action_feat)

        force_feat = action_feat_raw.unsqueeze(2)
        force_feat = F.relu(self.force_feat_block1(force_feat))
        force_feat = F.relu(self.force_feat_block2(force_feat))
        force_feat = F.relu(self.force_feat_block3(force_feat))
        force_feat4 = F.relu(self.force_feat_block4(force_feat))
        force_feat = torch.tanh(self.force_feat_block5(force_feat4)) * self.max_action

        output_stacked = force_feat
        if self.params.haptic_net_separate:
            haptic_feat = action_feat_raw.unsqueeze(2)
            haptic_feat = F.relu(self.haptic_feat_block1(haptic_feat))
            haptic_feat = F.relu(self.haptic_feat_block2(haptic_feat))
            haptic_feat = F.relu(self.haptic_feat_block3(haptic_feat))
            haptic_feat = F.relu(self.haptic_feat_block4(haptic_feat))
            haptic_feat = torch.cat([haptic_feat, force_feat4], dim=1)  # concatenate channels (intermediate repr)
            haptic_feat = F.relu(self.haptic_feat_block5_cat(haptic_feat))

            output_stacked = torch.cat([output_stacked, haptic_feat], dim=1)

        _, n_dim, timesteps = output_stacked.size()
        force = output_stacked.view(-1, n_dim * timesteps)
        print("stacked output size", force.size())
        force = F.tanh(force) * self.max_action
        if self.params.force_term:
            action = torch.cat([action, force], -1)
        # if self.params.only_force_term:
        #  action = force
        return action
