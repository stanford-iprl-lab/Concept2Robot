...
#      nn.BatchNorm2d(256),
    ...
    self.img_feat_block3 = nn.Linear(256, 128)
...
######
    self.goal_feat_block1 = nn.Linear(self.action_dim,64)
    self.goal_feat_block2 = nn.Linear(64, 128)
    self.goal_feat_block3 = nn.Linear(128, 128)
...
#####3 Force 
    ...
    self.force_feat_block5 = nn.Sequential(
      nn.ConvTranspose1d(in_channels=128,out_channels=self.params.a_dim,kernel_size=3,stride=2,padding=1,bias=False),
    )
    ...
    set_init([self.img_feat_block2, self.img_feat_block3, self.img_feat_block4, self.task_feat_block1, self.task_feat_block2, self.task_feat_block3,\
      ...
      self.goal_feat_block1, self.goal_feat_block2, self.goal_feat_block3])
...
###################################################################
    ...
    action = self.action_feat_block4(action_feat)
    #action = self.max_action * torch.tanh(action_feat)

###################################################################
    #goal_feat = F.relu(self.goal_feat_block1(task_feat))
    #goal_feat = F.relu(self.goal_feat_block2(goal_feat))
    #goal_feat = F.relu(self.goal_feat_block3(goal_feat))

    force_feat_raw = task_feat#torch.cat([goal_feat, action_feat_raw],-1)
    ...
    force = force_feat.view(-1, timesteps * n_dim)
