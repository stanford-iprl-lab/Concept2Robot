...
class Actor_F(nn.Module):
  def __init__(self, state_dim, action_dim, task_dim, max_action, params):
    ...
    self.img_feat_block3 = nn.Linear(256 * self.params.stack_num, 256)
    ...
    set_init([self.img_feat_block2, self.img_feat_block3, \
              self.task_feat_block1, self.task_feat_block2, self.task_feat_block3,\
  ...
  def forward(self, state, task_vec):
    ...
    img_feat = self.img_feat_block3(img_feat)
    ...
    coupling_feat = self.coupling_feat_block4(coupling_feat)
