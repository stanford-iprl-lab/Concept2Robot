...
class Worker(object):
    ...
    def test(self, restore_episode=0, restore_path=None):
        ...
        for ep_iter in range(max_iteration):
            ...
            while True:
                ...
                print("Tesing", "success", suc, "taskid", self.TaskId, "ep_", ep_, " action", action_clip[:7], "action_pred", action_pred[:7],
            ...
            while True:
                ...
                while not done:
                    ...
                    if total_timestep < self.params.start_learning_timestep:
                        feedback_pred = np.random.uniform(-0.5, 0.5,
                                                        size=(self.params.traj_timesteps + 1) * self.params.a_dim)
                    else:
                        feedback_pred = self.agent.choose_action_feedback(observation_feedback, self.task_vec)
                    #step(self, action=None, f_w=None, coupling=None, reset=True, test=False)
                    observation_next, reward, done, suc = self.env.step(np.zeros((7,)), None, feedback_term, reset_flag)
    ...
    def imitate(self):
        ...
        save_top_dir_dict[94] = "/juno/group/linshao/ConceptLearning/94" # yes
